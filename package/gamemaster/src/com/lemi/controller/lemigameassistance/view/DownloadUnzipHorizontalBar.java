 ...
 */
public class DownloadUnzipHorizontalBar extends ProgressBar {
  ...
  public DownloadUnzipHorizontalBar(Context context) {
    super(context);
    DownloadManager.getInstance().addVisibleListener(downloadListener);
    ZipManager.getInstance().addListener(unzipListener);
  }

  public DownloadUnzipHorizontalBar(Context context, AttributeSet attrs) {
    super(context, attrs);
    DownloadManager.getInstance().addVisibleListener(downloadListener);
    ZipManager.getInstance().addListener(unzipListener);
  }

  public DownloadUnzipHorizontalBar(Context context, AttributeSet attrs, int defStyle) {
  ...
  private void initStatus() {
    ...
    DataUtils.runAsyncTask(new AsyncTask<Void, Void, DownloadInfo>() {
      ...
      protected void onPostExecute(DownloadInfo info) {
        ...
        setStatus(DownloadUnzipHorizontalBar.Status.INVISIBLE);
      }
    });
  }
  ...
  private boolean checkPackageNameValid(String packageName) {
    if (TextUtils.isEmpty(DownloadUnzipHorizontalBar.this.packageName)) {
      return false;
    }
    return DownloadUnzipHorizontalBar.this.packageName.equals(packageName);
  }
