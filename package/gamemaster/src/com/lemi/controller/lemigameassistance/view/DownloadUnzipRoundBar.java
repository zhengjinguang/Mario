...
public class DownloadUnzipRoundBar extends RoundProgressBar {
  ...
  public DownloadUnzipRoundBar(Context context, AttributeSet attrs) {
    super(context, attrs);
    DownloadManager.getInstance().addVisibleListener(downloadListener);
    ZipManager.getInstance().addListener(unzipListener);
  }

  public DownloadUnzipRoundBar(Context context, AttributeSet attrs, int defStyle) {
  ...
  private void initStatus() {
    ...
    DataUtils.runAsyncTask(new AsyncTask<Void, Void, DownloadInfo>() {
      ...
      protected void onPostExecute(DownloadInfo info) {
        ...
        setStatus(DownloadUnzipRoundBar.Status.INVISIBLE);
      }
    });
  }
  ...
  private boolean checkPackageNameValid(String packageName) {
    if (TextUtils.isEmpty(DownloadUnzipRoundBar.this.packageName)) {
      return false;
    }
    return DownloadUnzipRoundBar.this.packageName.equals(packageName);
  }
